...
{key:"followme_mode",label:"Follow-Me Mode",description:"Stored FM mode preference on this RX. IT IS NOT AN AUTO-ARM SOURCE: the RX arms Follow-Me only from the TX's live 0xF2 mode declaration, and an undeclared mode means IDLE — it never falls back to this value (the old 0xFF fallback was removed because a factory RX would have booted latently armed). Kept so the setting round-trips through backup/restore and so the mode names stay documented in one place. 0=Disabled (RTM throttle-limit only, no steering), 1=Near Right (rear-right diagonal), 2=Behind (directly behind foiler, default), 3=Near Left (rear-left diagonal). Default 2.",group:"Follow-Me",type:"enum",def:2,min:0,max:3,options:[{v:0,l:"Disabled"},{v:1,l:"Near Right"},{v:2,l:"Behind (default)"},{v:3,l:"Near Left"}]},
...
{key:"tx_gps_stale_timeout_ms",label:"TX GPS Stale Timeout",description:"Maximum age of TX GPS data before the RX treats it as stale. DO NOT SET 0 — the test is (age > this), so 0 makes every fix stale instantly, which fails RTM Gate 4 AND Follow-Me condition 4 and leaves both autonomous modes permanently dead. This value cannot be disabled; the minimum useful setting is a few hundred ms. Default 3000ms (3 seconds). Raise it if the link drops packets, lower it if you want RTM/FM to give up sooner on a lagging TX. It also sets the freshness window the heading bootstrap uses.",group:"GPS",type:"int",def:3000,min:0,max:65535,unit:"ms"},

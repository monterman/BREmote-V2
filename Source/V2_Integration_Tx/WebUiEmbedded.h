// V2.5-Evo - 2026-08-25 - Follow-Me starting-mode selector extended with F4 In Front (max 4) and its already-ahead engagement warning. UI only; no confStruct/SW_VERSION change.
...
{key:"gps_en",label:"GPS Enabled",description:"0=GPS disabled (RTM and Follow-Me unavailable), 1=GPS active. Requires GPS module physically installed. Default 1.",group:"GPS",type:"bool",def:1,min:0,max:1},
{key:"followme_mode",label:"Follow-Me Starting Mode",description:"FM starting mode loaded on first arm this session. RAM override only — never written back to SPIFFS. 1=Near Right, 2=Behind (default), 3=Near Left, 4=In Front. In Front engages only after the buggy is already proven ahead of the rider.",group:"Follow-Me",type:"enum",def:2,min:1,max:4,options:[{v:1,l:"Near Right"},{v:2,l:"Behind (default)"},{v:3,l:"Near Left"},{v:4,l:"In Front"}]},
...
{key:"tx_gps_stale_timeout_ms",label:"TX GPS Stale Timeout",description:"How long a TX GPS fix is valid before considered stale. 0=never expires, 65535=very lenient. Lower values = stricter RTM GPS safety. Milliseconds. Default 3000.",group:"GPS",type:"int",def:3000,min:0,max:65535,unit:"ms"},
...
{key:"rtm_hold_duration_s",label:"RTM Arm Hold Time",description:"LEFT-hold (combo) duration to arm RTM. Live and tunable: 3s (fast) to 10s (very deliberate). Default 3. The 3s floor clears the 2s simple LEFT-hold (lock), so arming and locking stay distinct gestures.",group:"RTM",type:"int",def:3,min:3,max:10,unit:"s"},
{key:"rtm_arm_window_s",label:"RTM Arm Window",description:"Time to complete throttle squeeze(s) after the RTM arm gesture. 5s=very tight window (hard to complete), 30s=generous. Default 15.",group:"RTM",type:"int",def:15,min:5,max:30,unit:"s"},
{key:"rtm_double_squeeze_en",label:"RTM Double-Squeeze",description:"How RTM is armed once the arm gesture starts. 1 = double squeeze (default): the throttle must be squeezed TWICE — the deliberate gesture, harder to trigger by accident. 0 = a single squeeze held for ~500 ms — easier to perform, but easier to trigger unintentionally.",group:"RTM",type:"bool",def:1,min:0,max:1},

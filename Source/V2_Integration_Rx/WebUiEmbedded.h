...
{key:"gps_en",label:"GPS Enabled",description:"0=GPS module disabled (no UART polling, all GPS-dependent features blocked), 1=GPS enabled. RTM and Follow-Me require GPS enabled. Default 0.",group:"GPS",type:"bool",def:0,min:0,max:1},
{key:"gps_update_hz",label:"GPS Update Rate",description:"How many times per second the RX drains the GPS UART for new NMEA data. 2=500ms interval (default, safe for BN-880 at 5Hz), 5=200ms (poll every 200ms, catches all 5Hz sentences). Range 1-10 Hz. Does not change the GPS module output rate — only how often the firmware reads it.",group:"GPS",type:"int",def:2,min:1,max:10,unit:"Hz"},
{key:"gps_chip_type",label:"GPS Module Type",description:"GPS module type — determines init sequence, baud rate, and update rate. 0=BN-220 no compass, 1=BN-880 with compass (default for RX), 2=M10 no compass, 3=M10 with compass. Reboot required after change.",group:"GPS",type:"enum",def:1,min:0,max:3,options:[{v:0,l:"BN-220 no compass (9600→115200, 5Hz)"},{key:"gps_dyn_model",label:"GPS Dynamic Model",description:"u-blox navigation filter platform model. 0=default (Sea) — leave this unless you are above 500 m. 4=Automotive — REQUIRED for lakes above ~500 m altitude, because the Sea model has a 500 m ceiling and fixes degrade above it. 5=Sea (explicit) — best below 500 m: it constrains the filter to ~25 m/s and pins altitude near the surface, which sharpens course-over-ground, and COG is what Follow-Me steers on. Portable (0 in u-blox terms) is deliberately NOT offered — it permits 310 m/s and is what produced bogus 254 km/h high-confidence fixes.",group:"GPS",type:"enum",def:0,min:0,max:5,options:[{v:0,l:"Default (Sea)"},{v:4,l:"Automotive (above 500 m)"},{v:5,l:"Sea (explicit)"}]},{v:1,l:"BN-880 + compass (default, 9600→115200, 5Hz)"},{v:2,l:"M10 no compass (115200 direct, 10Hz, all constellations)"},{v:3,l:"M10 + compass (115200 direct, 10Hz, all constellations)"}]},
{key:"followme_mode",label:"Follow-Me Mode",description:"Persisted default FM mode (used as the 0xFF fallback when the TX has not sent a runtime override). 0=Disabled (RTM throttle-limit only, no steering), 1=Near Right (rear-right diagonal), 2=Behind (directly behind foiler, default), 3=Near Left (rear-left diagonal). Default 2.",group:"Follow-Me",type:"enum",def:2,min:0,max:3,options:[{v:0,l:"Disabled"},{v:1,l:"Near Right"},{v:2,l:"Behind (default)"},{v:3,l:"Near Left"}]},
{key:"kalman_en",label:"Kalman Filter",description:"0=Kalman filter disabled (raw GPS position used), 1=Kalman filter enabled (smooths GPS position noise for follow-me and RTM). Enable for autonomous use; disable to debug raw GPS output. Default 0.",group:"GPS",type:"bool",def:0,min:0,max:1},
{key:"tx_gps_stale_timeout_ms",label:"TX GPS Stale Timeout",description:"Maximum age of TX GPS data before RX considers it stale and blocks Phase B/C anti-spoofing checks. 0=disabled (never stale), 65535=65 seconds. Default 1000ms (1 second). Reduce if TX GPS updates fall behind.",group:"GPS",type:"int",def:1000,min:0,max:65535,unit:"ms"},
...
{key:"rtm_compass_required",label:"RTM Compass Required",description:"Requires a valid HEADING before RTM will arm - not specifically a compass, despite the name. It calls the same heading logic RTM steers on and accepts any source. 1=must have a heading to arm (default, recommended). 0=allow RTM to arm without one; steering simply does nothing until a heading appears. IMPORTANT: if you set Heading Source to 0 (GPS COG only) you MUST set this to 0 as well - with the compass disabled there is no heading while the buggy sits still, so arming would fail every time. The firmware will refuse to save that combination and tell you.",group:"RTM",type:"bool",def:1,min:0,max:1},
...
{key:"rtm_approach_zone_m",label:"RTM Approach Zone",description:"Distance from TX at which RTM begins ramping throttle down toward zero. Throttle reduces linearly from full at this outer edge to zero at the Hard Stop Distance — smooth arrival instead of sudden cut. 0=disabled (hard stop only, no ramp). Must be greater than Hard Stop Distance to have effect. Range 0-100 m, default 15 m.",group:"RTM",type:"int",def:15,min:0,max:100,unit:"m"},

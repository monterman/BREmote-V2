...
| File | SW | What it is |
|---|---|---|
| `BREmote-TX-SW27R3-gps-dyn-model.bin` | 27 | **Start here.** Everything in `SW27R2-haptics` **plus a selectable GPS dynamic model for the remote's own GPS.** The TX was hard-coded to the Sea model, which has a **500 m altitude ceiling** — above that, fixes degrade. Now settable, exactly as on the RX: `?set gps_dyn_model 4` then `?save`. Same `SW_VERSION` **27** and same **136-byte** `confStruct`, so **this flash does NOT reset your config** — throttle calibration and pairing are kept. **The reduced vibrations are now the standard behaviour**, not an experiment: a deliberate disarm and a mode cycle are silent, while every uncommanded stop still buzzes. |
| `BREmote-TX-SW27R2-haptics.bin` | 27 | The previous build, kept for rollback. Same reduced vibrations as above, without `gps_dyn_model`. |
| `BREmote-TX-SW27-ubx-checksum.bin` | 27 | Rollback to the **original vibration behaviour** — every stop and mode change buzzes. Everything in `gps-verified` **plus** the UBX checksum fix below. ⚠️ No `gps_dyn_model`, so if you ride above ~500 m you need the top row. |

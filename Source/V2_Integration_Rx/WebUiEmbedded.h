...
{key:"vesc_timeout_s",label:"VESC Timeout",description:"Seconds without a VESC UART packet before battery % and temperature show as unavailable. Default 6s. Range 5-60s. Note that a VESC cold restart takes ~8-9s, so at the 6s default the TX shows battery and temperature as unavailable for a moment while the VESC comes back — raise this to about 12s if you would rather it held the last reading across a restart.",group:"VESC",type:"int",def:6,min:5,max:60,unit:"s"},

...
{key:"fm_engage_dist_m",label:"FM Engage Distance",description:"LIVE. How far you must get from the buggy before Follow-Me is allowed to engage for the first time — the tow-rope interlock. This value IS the engage distance in metres; it is NOT the rope length. MEASURE YOUR OWN TOW ROPE and set this to at least one metre more than the rope length, so Follow-Me only engages once you have genuinely let go and separated. Example: a 20 ft (6.1 m) rope → set 8 m or more. A longer rope needs a bigger number. Valid entries are 0 = auto, or 8-50 m — there is no legal value between 0 and 8, and 8 m is the enforced minimum, not a recommendation. WHY: setting this at or below your rope length would let Follow-Me engage while you are still ON the rope, which is exactly what this interlock exists to prevent; the firmware rejects anything in between and clamps any older stored value up to 8 m. 0 = auto: the firmware computes it as 1.5 x (Min Distance + Smoothing Band). You must stay beyond this distance for 2 seconds before Follow-Me can engage.",group:"Follow-Me",type:"float",def:0,min:0,max:50,step:0.5,unit:"m"},

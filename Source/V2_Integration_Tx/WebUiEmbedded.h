...
const fields=[
{key:"radio_preset",label:"Radio Preset",description:"Radio frequency band. 1=EU 868 MHz, 2=US/AU 915 MHz. Only these two bands exist. Any invalid value safely defaults to EU868 instead of halting boot.",group:"Radio",type:"enum",def:1,min:1,max:2,options:[{v:1,l:"EU868"},{v:2,l:"US/AU915"}]},
...
{key:"followme_mode",label:"Follow-Me Starting Mode",description:"FM starting mode loaded on first arm this session. RAM override only — never written back to SPIFFS. 1=Near Right, 2=Behind (default), 3=Near Left.",group:"Follow-Me",type:"enum",def:2,min:1,max:3,options:[{v:1,l:"Near Right"},{v:2,l:"Behind (default)"},{v:3,l:"Near Left"}]},
...
{key:"rtm_hold_duration_s",label:"RTM Arm Hold Time",description:"LEFT-hold (combo) duration to arm RTM. Live and tunable: 3s (fast) to 10s (very deliberate). Default 5. The 3s floor clears the 2s simple LEFT-hold (lock), so arming and locking stay distinct gestures.",group:"RTM",type:"int",def:5,min:3,max:10,unit:"s"},
...
{key:"rtm_double_squeeze_en",label:"RTM Double-Squeeze",description:"RECOMMENDED: leave at 1 (double-squeeze). 1 = you must squeeze the throttle TWICE to engage RTM — the deliberate, harder-to-trigger-by-accident gesture, and the shipped default. 0 = a SINGLE squeeze engages RTM (hold ~500 ms) — easier, but easier to trigger unintentionally.",group:"RTM",type:"bool",def:1,min:0,max:1},
...
{key:"fm_hold_duration_s",label:"FM Hold Time",description:"RIGHT-hold (combo) duration to cycle Follow-Me mode. Live and tunable: 3s (fast) to 10s (very deliberate). Default 5. The 3s floor clears the 2s simple RIGHT-hold (display cycle), so the two gestures stay distinct.",group:"Follow-Me",type:"int",def:5,min:3,max:10,unit:"s"},
...
{key:"fm_arm_window_s",label:"FM Arm Window",description:"How long FM stays armed before silently auto-disarming if no throttle input. 10s=short window, 120s=2min, 600s=10min ('semi-expert' — one declaration covers most of a session, surviving long floats, slow takeoffs and swims). WARNING: expiry is SILENT (no display, no haptic), so with long windows you may believe you are armed when you are not. FM still cannot engage without the separation latch + held throttle + all conditions, so a long window cannot cause motion. Default 30.",group:"Follow-Me",type:"int",def:30,min:10,max:600,unit:"s"},
{key:"dist_unit",label:"Distance Unit",description:"Distance readout unit for the TX display (RTM, FM). NOTE: Feet is NOT yet rendered on the TX display this version — the display always shows metres regardless of this setting. The key is retained so a future version can implement feet. 0=Metres, 1=Feet.",group:"RTM",type:"enum",def:0,min:0,max:1,options:[{v:0,l:"Metres"},{v:1,l:"Feet (not yet rendered — shows metres)"}]},
...
{key:"mag_mode",label:"Magnet Gesture Mode",description:"OPTIONAL EXTRA HARDWARE — requires the DRV5032 Hall sensor fitted on GPIO 9. Leave at 0 if your remote does not have a magnet sensor; 0 changes nothing about how the remote behaves today. When enabled, hold a magnet against the case, wait for the buzz, then TAKE THE MAGNET AWAY to arm. The arming happens on magnet REMOVAL, not when the buzz fires — the buzz just means 'let go now and you will get this'. Mode 3 is two-tier: ONE buzz at 2s = release to arm Follow-Me, THREE buzzes at 5s = release to arm Return-To-Me. Modes 1 and 2 are single-tier: one buzz at 2s, release to arm that one mode. The confirm buzz after arming is always two fast buzzes, so the pattern tells you what you armed: 1-then-2 = Follow-Me, 3-then-2 = Return-To-Me. You do NOT need to let go of the throttle to arm - the gesture works mid-tow, and arming only declares intent, it never moves the craft on its own. Holds shorter than 2s do nothing (accident guard). The gesture only ARMS — it never disarms, and it does nothing if that mode is already armed. RTM also requires RTM Enabled + GPS; FM also requires FM Override + GPS.",group:"System",type:"enum",def:0,min:0,max:3,options:[{v:0,l:"0 - Off / sensor not fitted (default)"},{v:1,l:"1 - Magnet arms Follow-Me (2s)"},{v:2,l:"2 - Magnet arms Return-To-Me (2s)"},{v:3,l:"3 - Magnet arms FM (2s) / RTM (5s)"}]}
];

...
| File | SW | What it is |
|---|---|---|
| `BREmote-TX-SW27R3-gps-dyn-model.bin` | 27 | **Newest.** Everything in `SW27R2-haptics` **plus a selectable GPS dynamic model for the remote's own GPS.** The TX was hard-coded to the Sea model, which has a **500 m altitude ceiling** — above that, fixes degrade. Now settable, exactly as on the RX: `?set gps_dyn_model 4` then `?save`. Same `SW_VERSION` **27** and same **136-byte** `confStruct`, so **this flash does NOT reset your config** — throttle calibration and pairing are kept. ⚠️ **This build also carries the haptics cull**, because that work is already in the source. If you are on `ubx-checksum` and want the original vibration behaviour, this is not a drop-in — see the two rows below. |
...
| `BREmote-TX-SW27-ubx-checksum.bin` | 27 | **The recommended build if you want the original vibrations.** Everything in `gps-verified` **plus** the UBX checksum fix below. ⚠️ Does **not** have `gps_dyn_model` — if you ride above ~500 m you need the top row. |

...
{key:"tx_gps_stale_timeout_ms",label:"TX GPS Stale Timeout",description:"Maximum age of TX GPS data before the RX treats it as stale. This check CANNOT be disabled — the test is (age > this), so a very low value makes every fix stale instantly and would fail RTM Gate 4 AND Follow-Me condition 4, killing both autonomous modes. Values below 500ms are therefore clamped to 500ms. Default 3000ms (3 seconds). Raise it if the link drops packets, lower it if you want RTM/FM to give up sooner on a lagging TX. It also sets the freshness window the heading bootstrap uses.",group:"GPS",type:"int",def:3000,min:500,max:65535,unit:"ms"},
